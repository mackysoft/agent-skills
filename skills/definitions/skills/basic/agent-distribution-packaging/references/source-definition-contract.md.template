# Agent Distribution ソース定義契約

## 対象

この文書は、同じ配布物に含まれる Agent Distribution CLI が受け付けるソース入力の公開契約を定める。

ソース入力は、`bundle.json`、`definitions/skills` 以下のメタデータ、本文テンプレート、参照テンプレートで構成する。
...
- `bundle.json`、`definitions`、`definitions/skills` は必須である。
- `definitions/skills` には一つ以上のスキル定義を置く。
- `definitions/skills` 直下の各ディレクトリを category、その直下の各ディレクトリをスキル定義として読み取る。これより深いディレクトリはスキル定義として探索しない。
...
skill name は、category が異なる場合も含めて、同じ `definitions/skills` 全体で一意にする。`dependencies` とスキル選択には、このディレクトリ名をそのまま使う。
...
- 自分自身、重複、同じ `definitions/skills` に存在しない skill name を含めない。
- 依存関係全体に循環を作らない。
- 同じ `definitions/skills` にある別のスキルを、本文または参照内でドル記号を前置した skill name として参照する場合、その名前を `dependencies` に置く。宣言した依存先も、本文または参照内で同じ形式により参照する。
