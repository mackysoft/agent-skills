...
対象製品が管理する依存関係、ビルド設定、または生成コマンドから、AgentSkills を採用していることを確認する。
...
### Phase 3: 正規パッケージを用意する
追加または更新では、対象製品が用意する生成コマンドを使い、固定された AgentSkills バージョンで正規の出力先へ生成する。
レビューでは正規の出力先を変更せず、必要な場合は隔離した一時出力先へ生成して既存の生成物と照合する。
...
- 追加または更新では、完成したスキルの意味が正規ソース定義と生成物で保持され、古い生成物や再生成差分が残らず、製品での利用を検証している。
- レビューでは、正規の出力先を変更せず、成立している点、不整合、必要な差分、未検証範囲を返している。
