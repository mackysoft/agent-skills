...
file-set verifier は「期待された file があるか」「期待外の file があるか」「unsafe path / symlink / unsupported entry がないか」を扱う。expected file が存在し内容だけが異なる場合の digest 判定は、既存の `SkillInstalledContentDigestVerifier`、`SkillHostMaterializationInspector`、`SkillInstalledPackageIntegrityVerifier`、`SkillInstalledPackageDriftAnalyzer` の責務として維持する。
