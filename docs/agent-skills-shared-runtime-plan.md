...
> 状態: shared runtime への責務集約を記録した歴史的な計画です。現行 API の型と生成契約は実装を正本とし、バンドル運用は [Agent Skills 1.0 バンドル運用設計](agent-skills-bundle-1.0-plan.md) を参照してください。
...
product CLI は command 名、option 名、help 文言、exit code、product 固有 message、product の `CommandResult` 形式への最終変換を持つ。uCLI や dotmet の command 層で drift 判定、action count、host descriptor の規則、digest 検証、force semantics を再実装しない。
...
### 5. host descriptor
host adapter descriptor は、対応 host を `SkillHostKind` として保持し、導入と materialization に必要な方針を明示する。

- host
- project default target path
- user default target path
- user target root policy
- optional metadata artifact path
- reload guidance
...
- host literal を `ContractLiteralCodec` で `SkillHostKind` へ解決する。
...
5. host descriptor を追加し、list payload が descriptor から作れるようにする。
...
- product CLI 側に digest、drift、managed file set、force safety、host descriptor の規則、action count の判断が残っていない。
