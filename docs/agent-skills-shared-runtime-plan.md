...
## 責務境界
`MackySoft.AgentSkills` は SKILL package の生成、検証、materialization、install / update / uninstall / export / doctor の中立ロジックを持つ。

product CLI は command 名、option 名、help 文言、exit code、product 固有 message、product の `CommandResult` 形式への変換だけを持つ。uCLI や dotmet の command 層で drift 判定、action count、host capability、digest 検証、force semantics を再実装しない。
...
doctor と install / update / uninstall の state analysis は、次の drift を安定した failure code / drift kind として返す。
...
- unmanaged official skill name directory を force replace 可能にするかは、state と action を分けて明示する。
...
AgentSkills は次のような中立 DTO を返すか、既存 result から作れる projection service を持つ。
...
AgentSkills は product CLI が再利用できる parser / normalizer を提供する。

- required host を canonical host key へ解決する。
...
ただし、`--host`、`--scope`、`--repoRoot` などの option 名、必須扱いの UX、error message の最終文言は product CLI が決める。
...
2. managed file set verifier を構造化 result に拡張する。
3. install / update / uninstall / doctor の state analysis に manifest drift と file-set drift 詳細を組み込む。
