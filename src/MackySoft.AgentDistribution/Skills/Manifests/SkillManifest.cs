...
namespace MackySoft.AgentDistribution.Skills.Manifests;
...
public sealed class SkillManifest
{
    ...
    /// <param name="candidate"> The field-valid candidate whose supported-host membership has been validated. </param>
    ...
    /// <summary> Validates manifest-intrinsic candidates and creates canonical manifest instances. </summary>
    public sealed class Factory
    {
        ...
        private static AgentDistributionOperationResult<bool> ValidateShape (SkillManifestCandidate candidate)
        {
            ArgumentNullException.ThrowIfNull(candidate);

            var expectedHosts = Enum.GetValues<HostKind>().Order().ToArray();
