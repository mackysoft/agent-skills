...
public sealed class Sha256Digest : IEquatable<Sha256Digest>
{
    ...
    /// <summary> Completes an incremental SHA-256 hash and returns its canonical digest value. </summary>
    /// <param name="hash"> The incremental SHA-256 hash to complete and reset. </param>
    /// <returns> The computed digest. </returns>
    internal static Sha256Digest GetHashAndReset (IncrementalHash hash)
    {
        return new Sha256Digest(Sha256LowerHex.GetHashAndReset(hash));
    }
