...
internal sealed class AgentSkillsAgentsUninstallCommand
{
    private readonly AgentSkillsAgentsCommandRunner runner;
    private readonly IAgentSkillsCommandResultEmitter emitter;

    public AgentSkillsAgentsUninstallCommand (AgentSkillsAgentsCommandRunner runner, IAgentSkillsCommandResultEmitter emitter)
    {
    ...
    {
        var result = await runner.UninstallAsync(new AgentSkillsAgentUninstallCommandRequest(host, category, agent, scope, repositoryRoot, agentTargetDir, dryRun, force), cancellationToken).ConfigureAwait(false);
