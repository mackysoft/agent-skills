...
public sealed class SkillDoctorServiceTests
{
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-healthy");
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-host-conflict");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var installService = SkillTestData.CreateInstallService();
        var installResult = await installService.InstallAsync(
            packages[0].Manifest.CatalogId,
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", $"doctor-shared-{driftCase}");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-body-drift");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var installService = SkillTestData.CreateInstallService();
        var installResult = await installService.InstallAsync(
            packages[0].Manifest.CatalogId,
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-body-missing");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var installService = SkillTestData.CreateInstallService();
        var installResult = await installService.InstallAsync(
            packages[0].Manifest.CatalogId,
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-manifest-drift");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var installService = SkillTestData.CreateInstallService();
        var installResult = await installService.InstallAsync(
            packages[0].Manifest.CatalogId,
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-manifest-digest-drift");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var installService = SkillTestData.CreateInstallService();
        var installResult = await installService.InstallAsync(
            packages[0].Manifest.CatalogId,
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-reference-missing");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var installService = SkillTestData.CreateInstallService();
        var installResult = await installService.InstallAsync(
            packages[0].Manifest.CatalogId,
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-extra-empty-directory");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var installService = SkillTestData.CreateInstallService();
        var installResult = await installService.InstallAsync(
            packages[0].Manifest.CatalogId,
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-openai-metadata-drift");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var installService = SkillTestData.CreateInstallService();
        var installResult = await installService.InstallAsync(
            packages[0].Manifest.CatalogId,
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-openai-metadata-missing");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var installService = SkillTestData.CreateInstallService();
        var installResult = await installService.InstallAsync(
            packages[0].Manifest.CatalogId,
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-outdated");
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-openai-metadata-outdated");
        var installedPackages = await SkillTestData.GenerateFixturePackagesAsync();
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-claude-openai-metadata-outdated");
        var installedPackages = await SkillTestData.GenerateFixturePackagesAsync();
        var currentPackages = SkillTestData.ReplacePackage(
            installedPackages,
            SkillTestData.CreatePackageWithUpdatedOpenAiMetadata(installedPackages[0]));
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-invalid-manifest");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var targetRoot = scope.CreateDirectory(".agents/skills");
    ...
    public async Task DiagnoseAsync_ReportsPathUnsafe_WhenManifestSymlinkEscapesTarget ()
    {
        ...
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-manifest-symlink");
        using var outsideScope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-manifest-symlink-outside");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var targetRoot = scope.CreateDirectory(".agents/skills");
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "doctor-unsupported-host");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var doctor = SkillTestData.CreateDoctorService();
