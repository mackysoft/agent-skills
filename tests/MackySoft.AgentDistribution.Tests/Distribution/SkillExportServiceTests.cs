...
public sealed class SkillExportServiceTests
{
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "export-undefined-format");
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "export-empty-directory");
    ...
    public async Task ExportAsync_RejectsExistingSkillDirectoryThatEscapesThroughSymlink ()
    {
        ...
        using var outputScope = TestDirectories.CreateTempScope("agent-distribution-skills", "export-skill-symlink");
        using var outsideScope = TestDirectories.CreateTempScope("agent-distribution-skills", "export-skill-symlink-outside");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "export-zip-cleanup");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var service = SkillTestData.CreateExportService();
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "export-zip-deterministic");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var service = SkillTestData.CreateExportService();
    ...
    public async Task ExportAsync_ZipFormat_UsesOrdinalEntryOrderingForCultureSensitivePaths ()
    {
        ...
        try
        {
            ...
            using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "export-zip-ordinal-culture");
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "export-directory-zip-equivalence");
        var packages = await SkillTestData.GenerateFixturePackagesAsync();
        var service = SkillTestData.CreateExportService();
        ...
        HostKind host)
    {
        ...
        foreach (var package in packages.OrderBy(static package => package.Manifest.SkillName.Value, StringComparer.Ordinal))
        {
            ...
            foreach (var file in result.Value!.Files.OrderBy(static file => file.RelativePath.Value, StringComparer.Ordinal))
            {
