...
public sealed class CanonicalSkillPackageTextEncodingTests
{
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "package-reader-skill-bom");
        var package = (await SkillTestData.GenerateFixturePackagesAsync())[0];
        var skillsRoot = AbsolutePath.Parse(scope.GetPath("skills"));
        ...
        var skillPath = Path.Combine(skillsRoot.Value, package.Manifest.SkillName.Value, "SKILL.md");
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-distribution-skills", "package-reader-crlf");
        var package = (await SkillTestData.GenerateFixturePackagesAsync())[0];
        var skillsRoot = AbsolutePath.Parse(scope.GetPath("skills"));
        var writeResult = await SkillTestData.CreateCanonicalPackageWriter().WriteToStagingAsync(
            package,
