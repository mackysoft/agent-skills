...
namespace MackySoft.AgentDistribution.Tests.SkillDefinitions;
...
public sealed class SkillDefinitionSourceTests
{
    ...
    [
        "agent-distribution call --allowDangerous",
        "agent-distribution plan --allowDangerous",
        "agent-distribution validate --allowDangerous",
        "arbitrary C#",
        "execute C#",
        "run C#",
        "arbitrary shell",
    ...
    public void SkillBundle_HasExpectedSourceMetadata ()
    {
        ...
        Assert.Equal("com.mackysoft.agent-distribution", root.GetProperty("catalogId").GetString());
    ...
    public void SkillDefinitions_KeepInstructionGuardrails ()
    {
        ...
        foreach (var skillName in SkillTestData.ExpectedSkillNames)
        {
            ...
            Assert.Contains("agent-distribution ops describe <opName>", template, StringComparison.Ordinal);
    ...
    private static bool ContainsOperationCatalogTableCopy (string text)
    {
        ...
        while (reader.ReadLine() is { } line)
        {
            if (line.TrimStart().StartsWith("|", StringComparison.Ordinal)
                && line.Contains("agent-distribution.", StringComparison.OrdinalIgnoreCase))
            {
                operationTableRows++;
            }
        }
