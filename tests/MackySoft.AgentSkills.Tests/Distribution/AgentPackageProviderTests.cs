...
public sealed class AgentPackageProviderTests
{
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-skills-agent-package-provider", "all-agents");
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-skills-agent-package-provider", "exact-selection");
        var skills = await SkillTestData.GenerateFixturePackagesAsync();
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-skills-agent-package-provider", "unknown-category");
        var skills = await SkillTestData.GenerateFixturePackagesAsync();
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-skills-agent-package-provider", "category-name-mismatch");
        var skills = await SkillTestData.GenerateFixturePackagesAsync();
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-skills-agent-package-provider", "unknown-name");
        var skills = await SkillTestData.GenerateFixturePackagesAsync();
        await WriteBundleAsync(scope.FullPath, skills, [CreateAgent(skills, new AgentCategory("planning"), new AgentName("planner"), [])]);
        var provider = CreateProvider(scope.FullPath);
    ...
    {
        using var scope = TestDirectories.CreateTempScope("agent-skills-agent-package-provider", "missing-agent-skill-dependency");
        var skills = await SkillTestData.GenerateFixturePackagesAsync();
    ...
    private static AgentPackageProvider CreateProvider (string packageBaseDirectory)
    {
        ...
        var bundleReader = new CanonicalAgentSkillsBundleReader(
        ...
        IReadOnlyList<CanonicalAgentPackage> agents)
    {
        ...
        var descriptor = new AgentSkillsBundleDescriptor(
            ...
            new AgentSkillsBundleDigestCalculator(skillManifestSerializer, agentManifestSerializer, digestCalculator).ComputeDigest(skills, agents));
        var bundle = new CanonicalAgentSkillsBundle(descriptor, skills, agents);
        var agentManifestDigestCalculator = new AgentManifestDigestCalculator(agentManifestSerializer);
        var agentReader = new CanonicalAgentPackageReader(
            agentManifestSerializer,
            digestCalculator,
