...
public sealed class AgentSkillsBundleBuildServiceTests
{
    ...
    [Fact]
    [Trait("Size", "Small")]
    public async Task BuildAsync_WhenAgentReferenceIsUndeclared_FailsBeforeWriting ()
    {
        using var scope = TestDirectories.CreateTempScope("agent-skills-v2", "undeclared-reference");
        WriteMixedSource(scope, instructions: "Use $example-skill before producing the design.\n", dependency: null);
        var result = await AgentSkillsBundleBuildService.CreateDefault().BuildAsync(scope.FullPath, null, check: false, CancellationToken.None);
    ...
    [Fact]
    [Trait("Size", "Small")]
    public async Task BuildAsync_WhenBuiltInAgentOverrideIsMissing_FailsBeforeWriting ()
    {
        using var scope = TestDirectories.CreateTempScope("agent-skills-v2", "missing-built-in-override");
        ...
        Assert.False(result.IsSuccess);
        Assert.Equal(SkillFailureCodes.SourceInvalid, result.Failure!.Code);
        Assert.Contains("overridesBuiltIn=true", result.Failure.Message, StringComparison.Ordinal);
        Assert.False(Directory.Exists(scope.GetPath("generated")));
    }
        ...
        string agentName = "architect")
    {
        ...
        scope.WriteFile(
            ...
            instructions ?? "Use $example-skill before producing the design.\n");
        scope.WriteFile(
