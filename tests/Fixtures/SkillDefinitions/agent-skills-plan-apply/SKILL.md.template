...
2. For every primitive operation in the request, run `agent-skills ops describe <opName>` and use that runtime contract.
3. Build the smallest JSON request that expresses the intended context, selection, action, and commit boundary.
4. Run `agent-skills validate` before any Unity-backed execution.
5. Run `agent-skills plan` and inspect the plan evidence before mutating.
6. Run `agent-skills call` only after the plan is acceptable, using a plan token when available or required.
7. Run `agent-skills verify --profile built-in:mutation --from <result.json>` after mutation, then add targeted reads, tests, or logs only when the claim packet requires them. Use `built-in:script` for C# script changes and omit `--profile` only when default project-level verification is intended.
...
- Do not use fixed sleep while waiting for readiness. Let Agent Skills lifecycle and timeout results drive the next step.
...
- Use `agent-skills codes describe <CODE>` for machine-readable code meaning; do not branch on free-form messages.
