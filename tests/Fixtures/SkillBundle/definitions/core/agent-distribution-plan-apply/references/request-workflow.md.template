...
Use this reference when constructing or applying a Agent Distribution JSON request.
...
3. Run `agent-distribution ops describe <opName>` for each primitive operation being used.
...
## Execution
1. Run `agent-distribution validate` and fix static contract errors first.
2. Run `agent-distribution plan` to preview targets, changed state, touched contexts, and plan token behavior.
3. Run `agent-distribution call` only after reviewing the plan.
