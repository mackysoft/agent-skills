...
Use this reference when deciding how to prove a Unity change after Agent Distribution execution.
...
- `agent-distribution query` and `agent-distribution resolve`: targeted follow-up observation.
- `agent-distribution test run`: Unity Test Framework proof when behavior depends on tests.
- `agent-distribution logs unity read` and `agent-distribution logs daemon read`: lifecycle, compile, reload, IPC, and runtime diagnostics.
