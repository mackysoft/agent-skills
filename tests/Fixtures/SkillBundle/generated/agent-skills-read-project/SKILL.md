...
## Workflow
1. Resolve the target project with `agent-skills status` or an explicit `--projectPath`.
2. Read the current state with the narrowest useful `agent-skills query` or `agent-skills resolve` command.
3. Before depending on any primitive operation, run `agent-skills ops describe <opName>` and treat that output as the current contract.
...
- Do not use fixed sleep as a readiness strategy. Use Agent Skills lifecycle, status, logs, and command results.
