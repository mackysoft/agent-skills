...
Use this skill to inspect a Unity project with Agent Distribution before deciding what to edit or verify.

## Workflow
1. Resolve the target project with `agent-distribution status` or an explicit `--projectPath`.
2. Read the current state with the narrowest useful `agent-distribution query` or `agent-distribution resolve` command.
3. Before depending on any primitive operation, run `agent-distribution ops describe <opName>` and treat that output as the current contract.
...
- Do not use fixed sleep as a readiness strategy. Use Agent Distribution lifecycle, status, logs, and command results.
