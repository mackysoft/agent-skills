...
Use this skill to diagnose Agent Distribution execution, daemon, lifecycle, readIndex, timeout, and log problems.

## Workflow
1. Identify the command, project path, mode, timeout, and exact code value.
2. Use `agent-distribution status`, daemon commands, readIndex output, and logs to locate the failing boundary.
3. Use `agent-distribution codes describe <CODE>` to read the static meaning of machine-readable codes.
4. If an operation-specific result is involved, run `agent-distribution ops describe <opName>` before interpreting its contract.
...
- Do not use fixed sleep to wait out compile, reload, or daemon readiness. Use lifecycle-aware Agent Distribution commands and bounded timeouts.
